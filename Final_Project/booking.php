...
if (isset($_POST['book'])) {
    ...
    // Check if the same sport, date, time, and location are already booked
    $check_query = "SELECT * FROM bookings WHERE sport = ? AND date = ? AND time = ? AND location = ?";
    $stmt = mysqli_stmt_init($conn);
    
    if (mysqli_stmt_prepare($stmt, $check_query)) {
        mysqli_stmt_bind_param($stmt, "ssss", $sport, $date, $time, $location);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);

        if (mysqli_num_rows($result) > 0) {
            // Conflict found, display a message
            echo "<div class='alert alert-danger'>This slot is already booked. Please choose another time or date.</div>";
        } else {
            // No conflict, insert the booking data into the database
            $insert_query = "INSERT INTO bookings (user_id, sport, date, time, location) VALUES (?, ?, ?, ?, ?)";
...
<body>
    ...
    <div class="container">
        ...
        <form action="booking.php" method="post">
            ...
            <div class="form-group" style="margin-bottom: 15px;">
                <label for="location">Select Sports Complex:</label>
                <select name="location" class="form-control" id="location" required>
                    <?php foreach ($sports_complexes as $complex): ?>
                        <option value="<?php echo $complex; ?>"><?php echo $complex; ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
